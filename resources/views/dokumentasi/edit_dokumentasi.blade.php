...
<div class="bg-white rounded-3 p-4" style="margin-left: 260px; margin-top: 75px; height: auto;">
    <div class="card p-4 shadow-lg border-0">
        <div class="card-body">
            <h2 class="h5 fw-semibold text-secondary mb-4">Edit Dokumentasi</h2>
            ...
            <form action="{{ route('dokumentasi.update', $dokumentasi->id_dokumentasi) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <input type="hidden" name="id_rapat" value="{{ $dokumentasi->id_rapat }}">
                ...
                <div class="mb-3">
                    ...
                    <textarea id="editor" name="deskripsi" class="form-control">{!! old('deskripsi', $dokumentasi->deskripsi) !!}</textarea>
                </div>

                <!-- Gambar Lama -->
                @if ($dokumentasi->files->count())
                <div class="mb-3">
                    <label class="form-label fw-semibold">Gambar Lama</label>
                    <div class="d-flex flex-wrap gap-2">
                        @foreach ($dokumentasi->files as $file)
                            <img src="{{ asset('storage/' . $file->file_path) }}" alt="File Lama" class="rounded border" width="50" height="50" style="object-fit: cover;">
                        @endforeach
                    </div>
                </div>
                @endif
                ...
                <div class="mb-4">
                    ...
                    <div class="border border-dashed border-secondary-subtle rounded bg-light p-3">
                        <p class="text-muted small fst-italic">
                            Upload gambar kotak, maksimal 100KB
                        </p>

                        <div class="d-flex align-items-center gap-3">
                            @if($dokumentasi->file_path && count($dokumentasi->file_path) > 0)
                                <img src="{{ asset('storage/' . $dokumentasi->file_path[0]) }}" alt="Preview" id="previewImage" class="rounded" width="50" height="50">
                            @else
...
<!-- TinyMCE & JS sama seperti sebelumnya -->
